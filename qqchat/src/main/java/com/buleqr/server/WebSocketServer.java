package com.buleqr.server;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import com.baomidou.mybatisplus.extension.exceptions.ApiException;
import com.buleqr.config.CustomSpringConfigurator;
...
public class WebSocketServer {
    ...
    @Autowired
    public void setDeviceListenerService(MessageListService messageListService) {
        WebSocketServer.messageListService = messageListService;
    }
...
//    private static CopyOnWriteArraySet<WebSocketServer> webSocketSet = new CopyOnWriteArraySet<>();
    ...
    public void onOpen(Session session, @PathParam("userId") String userId) {
        ...
        try {
            AjaxResult ajaxResult = new AjaxResult(10200,"连接成功");
            sendMessage(JSON.toJSONString(ajaxResult));
        } catch (IOException e) {
            e.printStackTrace();
            throw new ServiceException("连接失败",10500);
        }
    ...
    public void onMessage(String message, Session session) {
        ...
        if (StringUtils.isNotBlank(message)) {
            try {
                ...
                JSONObject messageDetail = jsonObject.getJSONObject(AjaxResult.DATA_TAG);
                String toUserId = messageDetail.getString("toUserId");
    ...
    public void onError(Session session, Throwable error) {
        ...
        throw new ServiceException(error.getMessage(),400);
